"""
00n_sex_stratification: CREATE SEX-STRATIFIED MORTALITY FILES
================================================================
Process SIM mortality data and create sex-stratified files for Phase 4 analysis.
...
print("="*70)
print("00n_sex_stratification: CREATE SEX-STRATIFIED MORTALITY FILES")
print("="*70)
...
intermediate_data = {sex: [] for sex in sex_groups.keys()}
immediate_data = {sex: [] for sex in sex_groups.keys()}
...
for i, file in enumerate(mort_files, 1):
    ...
    try:
        ...
        required_cols = ['DTOBITO', 'CODMUNRES', 'IDADE', 'SEXO']
        ...
        # Aggregate by sex for each region level
        for sex_code, sex_value in sex_groups.items():
            # Intermediate
            df_sex_inter = df_inter[df_inter['sex'] == sex_value]
            if len(df_sex_inter) > 0:
                agg_inter = df_sex_inter.groupby(['date', 'intermediate_code']).size().reset_index(name='deaths')
                agg_inter.columns = ['date', 'region_code', 'deaths']
                agg_inter['region_code'] = agg_inter['region_code'].astype(int)
                intermediate_data[sex_code].append(agg_inter)
            
            # Immediate
            df_sex_immed = df_immed[df_immed['sex'] == sex_value]
            if len(df_sex_immed) > 0:
                agg_immed = df_sex_immed.groupby(['date', 'immediate_code']).size().reset_index(name='deaths')
                agg_immed.columns = ['date', 'region_code', 'deaths']
                agg_immed['region_code'] = agg_immed['region_code'].astype(int)
                immediate_data[sex_code].append(agg_immed)
...
# Combine and save
print("\n[3] Combining and saving sex-stratified files...")

for sex_code, sex_value in sex_groups.items():
    sex_label = sex_code.capitalize()
    
    # Intermediate
    if intermediate_data[sex_code]:
        df_combined = pd.concat(intermediate_data[sex_code], ignore_index=True)
        ...
        output_file = OUTPUT_DIR / f'mortality_intermediate_daily_{sex_code}.parquet'
        df_full.to_parquet(output_file, index=False)
        
        print(f"  {sex_label}: {output_file.name}")
        print(f"    Regions: {len(regions)}, Records: {len(df_full):,}, Deaths: {df_full['deaths'].sum():,}")
    
    # Immediate
    if immediate_data[sex_code]:
        df_combined = pd.concat(immediate_data[sex_code], ignore_index=True)
        df_combined = df_combined.groupby(['date', 'region_code'])['deaths'].sum().reset_index()
        ...
        output_file = OUTPUT_DIR / f'mortality_immediate_daily_{sex_code}.parquet'
        df_full.to_parquet(output_file, index=False)
