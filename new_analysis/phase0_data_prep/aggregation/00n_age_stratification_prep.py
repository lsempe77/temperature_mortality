"""
00n_age_stratification: CREATE AGE-STRATIFIED MORTALITY FILES
================================================================
Process SIM mortality data and create age-stratified files for Phase 4 analysis.

Age Groups:
- 60-69 years (young elderly)
- 70-79 years (middle elderly)  
- 80+ years (oldest old)
...
print("="*70)
print("00n_age_stratification: CREATE AGE-STRATIFIED MORTALITY FILES")
print("="*70)
...
intermediate_data = {age: [] for age in age_groups.keys()}
immediate_data = {age: [] for age in age_groups.keys()}
...
for i, file in enumerate(mort_files, 1):
    ...
    try:
        ...
        required_cols = ['DTOBITO', 'CODMUNRES', 'IDADE']
        ...
        # Aggregate by age group for each region level
        for age_code, (min_age, max_age) in age_groups.items():
            # Intermediate
            df_age_inter = df_inter[(df_inter['age'] >= min_age) & (df_inter['age'] <= max_age)]
            if len(df_age_inter) > 0:
                agg_inter = df_age_inter.groupby(['date', 'intermediate_code']).size().reset_index(name='deaths')
                agg_inter.columns = ['date', 'region_code', 'deaths']
                agg_inter['region_code'] = agg_inter['region_code'].astype(int)
                intermediate_data[age_code].append(agg_inter)
            
            # Immediate
            df_age_immed = df_immed[(df_immed['age'] >= min_age) & (df_immed['age'] <= max_age)]
            if len(df_age_immed) > 0:
                agg_immed = df_age_immed.groupby(['date', 'immediate_code']).size().reset_index(name='deaths')
                agg_immed.columns = ['date', 'region_code', 'deaths']
                agg_immed['region_code'] = agg_immed['region_code'].astype(int)
                immediate_data[age_code].append(agg_immed)
...
# Combine and save
print("\n[3] Combining and saving age-stratified files...")

for age_code, (min_age, max_age) in age_groups.items():
    age_label = f"{min_age}-{max_age}+" if max_age >= 150 else f"{min_age}-{max_age}"
    
    # Intermediate
    if intermediate_data[age_code]:
        df_combined = pd.concat(intermediate_data[age_code], ignore_index=True)
        ...
        output_file = OUTPUT_DIR / f'mortality_intermediate_daily_{age_code}.parquet'
        df_full.to_parquet(output_file, index=False)
        
        print(f"  {age_label}: {output_file.name}")
        print(f"    Regions: {len(regions)}, Records: {len(df_full):,}, Deaths: {df_full['deaths'].sum():,}")
    
    # Immediate
    if immediate_data[age_code]:
        df_combined = pd.concat(immediate_data[age_code], ignore_index=True)
        df_combined = df_combined.groupby(['date', 'region_code'])['deaths'].sum().reset_index()
        ...
        output_file = OUTPUT_DIR / f'mortality_immediate_daily_{age_code}.parquet'
        df_full.to_parquet(output_file, index=False)
